...
import jp.techacademy.yuu.funakoshi.apiapp.databinding.RecyclerFavoriteBinding
...
class ApiItemViewHolder(private val binding: RecyclerFavoriteBinding) :
    ...
    fun bind(shop: Shop, position: Int, adapter: ApiAdapter) {
        binding.rootView.apply {
            ...
            setOnClickListener {
                adapter.onClickItem?.invoke(if (shop.couponUrls.sp.isNotEmpty()) shop.couponUrls.sp else shop.couponUrls.pc,if (shop.couponUrls.sp.isNotEmpty()) shop.couponUrls.sp else shop.couponUrls.pc)
            }
        }
        ...
        binding.addressTextView.text = shop.address // 住所を表示
